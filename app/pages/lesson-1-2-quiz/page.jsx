...
const EVENTS = [
  {
    id:1, icon:"🚗", tag:"Universal", tagColor:T.slate,
    title:"Car Breakdown",
    ...
    bill:180000, billLabel:"Repair bill", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund",    icon:"🛸", correct:true,  needsTank:true,
        explain:"Exactly right. This is what the Emergency Fund is for. Colony untouched and still compounding." },
      { label:"Sell some investments",    icon:"🚀", correct:false,
        explain:"¥180,000 withdrawn from the colony today costs you " + c20(180000) + " in 20-year compounding. The colony doesn't get torn down for a car repair." },
      { label:"Put it on a credit card", icon:"💳", correct:false,
        explain:"Credit card interest at 18% adds ¥32,000 in year one alone. You'd be rebuilding a debt boss you already defeated in Level 0." },
    ],
  ...
  {
    id:2, icon:"🦷", tag:"Universal", tagColor:T.slate,
    ...
    bill:95000, billLabel:"Dental bill", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund",      icon:"🛸", correct:true,  needsTank:true,
        explain:"Perfect. Health emergencies always take priority — and your colony didn't lose a single brick." },
      { label:"Sell some investments",      icon:"🚀", correct:false,
        explain:"¥95,000 from the colony costs you " + c20(95000) + " over 20 years. Health emergencies are exactly what the tank is for." },
      { label:"Delay treatment to save up", icon:"⏳", correct:false,
        explain:"Delaying dental care turns a ¥95,000 problem into a ¥300,000+ extraction and implant — plus time off work. The tank exists so you never delay a health decision because of money." },
    ],
  ...
  {
    id:3, icon:"🚙", tag:"Japan — Shakken", tagColor:T.amber,
    ...
    bill:120000, billLabel:"Shakken + repairs", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund",      icon:"🛸", correct:true,  needsTank:true,
        explain:"Good. Shakken is predictable — every 2 years. Your Emergency Fund covers it cleanly. Ideally it has its own sinking fund over time." },
      { label:"Sell some investments",      icon:"🚀", correct:false,
        explain:"Shakken is a known, recurring expense. Selling colony bricks for something this predictable is a sequencing error that costs you " + c20(120000) + " over 20 years." },
      { label:"Skip it and keep driving",   icon:"🚫", correct:false,
        explain:"Driving without a valid shakken is illegal in Japan. Your insurance becomes void and fines can exceed the inspection cost itself. There is no skipping this one." },
    ],
  ...
  {
    id:4, icon:"📉", tag:"Market Event", tagColor:T.blue,
    ...
    bill:null, billLabel:null, isMarket:true,
    choices:[
      { label:"Do nothing — stay the course",       icon:"🧘", correct:true,
        explain:"This is the rule. An 18% dip is a normal quarterly chapter. Your oxygen tank means you don't need to sell. History shows it always recovers." },
      { label:"Sell now to stop the bleeding",      icon:"📉", correct:false,
        explain:"Selling during a dip converts a temporary paper loss into a permanent capital loss. Every single historical dip has recovered. You just need to not need the money right now — which is exactly why the tank exists." },
      { label:"Wait for the bottom, then rebuy",   icon:"⏳", correct:false,
        explain:"Nobody has ever reliably timed the bottom — not retail investors, not hedge funds. Studies show missing just the 10 best market days in a decade cuts returns in half. The rule is simple: do nothing." },
    ],
  ...
  {
    id:5, icon:"✈️", tag:"Universal", tagColor:T.slate,
    ...
    bill:280000, billLabel:"Travel + lost wages", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund", icon:"🛸", correct:true,  needsTank:true,
        explain:"Right. Emotional emergencies are still emergencies. Your oxygen tank absorbs this. You focus on family — not finances." },
      { label:"Sell some investments",  icon:"🚀", correct:false,
        explain:"¥280,000 from the colony costs " + c20(280000) + " over 20 years. Your tank exists so money never competes with family in your worst moments." },
      { label:"Take a personal loan",   icon:"🏦", correct:false,
        explain:"A personal loan at 5–8% adds financial stress on top of emotional stress — and creates a new debt obligation right when your income may already be disrupted by time off work." },
    ],
  ...
  {
    id:6, icon:"💼", tag:"Universal", tagColor:T.slate,
    ...
    bill:null, billLabel:null, isMarket:false, isDynamic:true,
    dynamicBillFn:(expenses) => expenses * 2,
    dynamicLabel:"2 months living expenses",
    choices:[
      { label:"Use my Emergency Fund", icon:"🛸", correct:true,  needsTank:true,
        explain:"This is the core use case. Job loss is the #1 reason you built 3–6 months of emergency cash. Colony stays untouched and keeps compounding." },
      { label:"Sell some investments",  icon:"🚀", correct:false,
        explain:"Job losses and market downturns are correlated — they often happen simultaneously. Selling your colony during a downturn means selling at the lowest point. That's double damage you can never undo." },
      { label:"Max out credit cards",   icon:"💳", correct:false,
        explain:"Credit card debt at 18% on 2 months of expenses costs tens of thousands per year in interest. You'd be rebuilding the exact debt boss from Level 0 at the worst possible time." },
    ],
  ...
  {
    id:7, icon:"🏠", tag:"Universal", tagColor:T.slate,
    ...
    bill:350000, billLabel:"Deposit + first/last month", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund", icon:"🛸", correct:true,  needsTank:true,
        explain:"Correct — though note: moving costs are predictable. Ideally they have their own sinking fund. Emergency Fund covers it for now, but plan ahead for next time." },
      { label:"Sell some investments",  icon:"🚀", correct:false,
        explain:"¥350,000 from the colony costs " + c20(350000) + " over 20 years. Moving costs are predictable enough to plan for — they belong in a sinking fund, not funded by colony bricks." },
      { label:"Borrow from family",     icon:"👨‍👩‍👦", correct:false,
        explain:"Borrowing from family adds relationship risk to financial stress. When repayment gets delayed — and it often does — it creates tension that outlasts the financial problem itself." },
    ],
  ...
  {
    id:8, icon:"📊", tag:"Japan — Tax Season", tagColor:T.amber,
    ...
    bill:85000, billLabel:"Tax underpayment", isMarket:false,
    choices:[
      { label:"Use my Emergency Fund",          icon:"🛸", correct:true,  needsTank:true,
        explain:"Right call. Tax surprises in Japan are common for expats — especially the first few years. Emergency Fund absorbs this in 24 hours. Colony intact." },
      { label:"Sell some investments",           icon:"🚀", correct:false,
        explain:"¥85,000 from the colony costs " + c20(85000) + " over 20 years. This is a classic oxygen-tank-sized expense — small enough to feel manageable, but the compounding cost is real." },
      { label:"Request a payment extension",     icon:"📋", correct:false,
        explain:"Japan's tax office expects on-time payment. Extensions can trigger penalties and interest that exceed the original underpayment amount. Your tank handles this cleanly." },
    ],
  ...
  {
    id:9, icon:"🚀", tag:"Market Event", tagColor:T.blue,
    ...
    bill:null, billLabel:null, isMarket:true, isGreed:true,
    choices:[
      { label:"Do nothing — protect the rule",    icon:"🧘", correct:true,
        explain:"Discipline wins. Your colony is already up 22%. Moving the oxygen tank into the market means the next emergency destroys the colony instead of being absorbed by the tank." },
      { label:"Move Emergency Fund into market",   icon:"📈", correct:false,
        explain:"The moment your tank is empty, every future emergency becomes a forced colony sell. One medical bill, one job shock — and you're selling investments at whatever price the market offers that day." },
      { label:"Take a loan to invest more",        icon:"💸", correct:false,
        explain:"Leveraged investing amplifies losses as much as gains. A 22% gain on borrowed money feels incredible. A 22% loss on borrowed money means you owe more than your portfolio is worth." },
    ],
  ...
  {
    id:10, icon:"🌊", tag:"Final Event — Japan", tagColor:T.red,
    ...
    bill:1200000, billLabel:"Repairs + temporary housing", isMarket:false, isBoss:true,
    choices:[
      { label:"Emergency Fund + don't sell investments", icon:"🛸", correct:true,  needsTank:true,
        explain:"Right instinct. Your Emergency Fund covers what it can. The investments stay untouched — even here. The gap between your tank and this bill is exactly what Level 4 is about." },
      { label:"Sell investments to cover the gap",      icon:"🚀", correct:false,
        explain:"Even at ¥1.2M, selling the colony is not the answer. The gap between your tank and this bill is what income protection insurance exists to cover — which is exactly what Level 4 teaches." },
      { label:"Take a large personal loan",             icon:"🏦", correct:false,
        explain:"A large loan at your most financially vulnerable moment creates a debt burden right when your income may be disrupted by displacement. This is insurance territory — not loan territory." },
    ],
    jessica:"",
  },
];
...
export default function QuizApp() {
  ...
  const pick = (choice) => {
    ...
    if (!isCorrect && newLives <= 0) setGameOver(true);
  };

  const next = () => {
    if (gameOver || isLast) { setScreen("result"); return; }
    const nextIdx = idx + 1;
    setIdx(nextIdx);
    setChosen(null);
  ...
  const restart = () => {
    setScreen("partA"); setPqIdx(0); setPScore(0); setPChosen(null); setPersonality(null);
    setIncome(350000); setExpenses(200000); setIdx(0); setLives(3); setScore(0);
    setTank(0); setStartTank(0); setChosen(null); setHistory([]);
    setGameOver(false); setShowTimestamp(true); setMonthsElapsed(0);
  };

  // ══════════════════════════════════════════════════════════
  // PART A — silent personality check
  // ══════════════════════════════════════════════════════════
  if (screen === "partA") {
      ...
      <Wrap>
        ...
        <Box>
          ...
          {q.choices.map((c,i) => {
            ...
            return (
              <button key={i} onClick={() => pickPQ(c)} disabled={!!pChosen} style={{
                width:"100%",
                background: isPicked ? T.teal + "25" : T.navyDeep,
                border:"2px solid " + (isPicked ? T.teal : T.navyMid),
                borderRadius:12, padding:"13px 14px", marginBottom:8,
                display:"flex", alignItems:"center", gap:10,
                cursor: pChosen ? "default" : "pointer", textAlign:"left", transition:"all 0.2s",
              }}>
          ...
          {pChosen && (
            <Btn color={T.teal} onClick={nextPQ}>
              {pqIdx < PERSONALITY_QS.length - 1 ? "Next Question →" : "See My Saving Profile →"}
            </Btn>
          )}
        </Box>
      </Wrap>
      ...
      <Wrap>
        ...
        <Box borderColor={p.color}>
          ...
          <div style={{ background:p.color + "12", border:"1px solid " + p.color + "40", borderRadius:12, padding:"12px 14px", marginBottom:14 }}>
            <div style={{ fontSize:12, color:T.offWhite, lineHeight:1.65 }}>
              Your saving personality determines how fast your Emergency Fund refills between life events. The stronger your saving instinct, the more resilient your tank.
            </div>
          </div>
  ...
  if (screen === "partB") {
    ...
    return (
      <Wrap>
        ...
        <Box>
          <div style={{ fontSize:13, color:T.offWhite, lineHeight:1.65, marginBottom:18 }}>
            Your simulation is personalised. Set your monthly income and expenses — your Emergency Fund and refill rate are calculated from your real numbers.
          </div>
          <SliderField label="Monthly take-home income" value={income} onChange={setIncome}
            min={150000} max={1000000} step={10000} display={yen(income)} color={T.teal} />
          <SliderField label="Monthly living expenses" value={expenses} onChange={setExpenses}
            min={80000} max={600000} step={10000} display={yen(expenses)} color={T.slate} />
    ...
    return (
      <Wrap>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <Btn color={T.teal} style={{ flex:1 }} onClick={restart}>Run Again</Btn>
          <Btn color={T.navyCard} style={{ flex:1, border:"1px solid " + T.slate + "44" }} onClick={() => alert("Lesson 1-1 unlocked!")}>Next Lesson →</Btn>
        </div>
      </Wrap>
    );
  }
      ...
      <div style={{ display:"flex", gap:10, marginBottom:14 }}>
        <div style={{ flex:2, background:T.navyCard, borderRadius:10, padding:"10px 12px", border:"1px solid " + T.navyMid }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
            ...
            <span style={{ fontSize:12, fontWeight:800, color: tankEmpty ? T.red : tank > startTank*0.3 ? T.oxygen : T.amber }}>
              {tankEmpty ? "EMPTY" : yen(tank)}
            </span>
          </div>
      ...
      {gameOver && chosen && (
        <Box borderColor={T.red}>
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:13, color:T.offWhite, lineHeight:1.6, marginBottom:14 }}>
              You ran out of lives. The bucket rule was broken too many times.
            </div>
      ...
      {!showTimestamp && !gameOver && (
        <Box borderColor={evBorderColor}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
            ...
            <div>
              <span style={{ fontSize:9, fontWeight:700, color:ev.tagColor, border:"1px solid " + ev.tagColor + "50", borderRadius:4, padding:"2px 6px", textTransform:"uppercase", letterSpacing:0.8 }}>
                {ev.tag}
              </span>
              {ev.isBoss && (
                <span style={{ fontSize:9, fontWeight:700, color:T.red, border:"1px solid " + T.red + "50", borderRadius:4, padding:"2px 6px", marginLeft:6, textTransform:"uppercase", letterSpacing:0.8 }}>
                  Final Event
                </span>
              )}
          ...
          {tankEmpty && evBill && !chosen && (
            <div style={{ background:T.red + "18", border:"1px solid " + T.red + "50", borderRadius:10, padding:"12px 14px", marginBottom:14 }}>
              ...
              <div style={{ fontSize:12, color:T.offWhite, lineHeight:1.5 }}>
                Earlier decisions drained your Emergency Fund. This is the moment the bucket rule catches up with you.
              </div>
            </div>
          )}
              ...
              {ev.choices.map((c,i) => {
                ...
                return (
                  <button key={i} onClick={() => !isLocked && pick(c)} style={{
                    ...
                    textAlign:"left", opacity: isLocked ? 0.35 : 1,
                  }}>
          ...
          {chosen && (
            <div>
              {/* Result — shows explanation specific to the choice made */}
              ...
              {ev.isBoss && (
                <div style={{ background:T.purple + "18", border:"2px solid " + T.purple + "50", borderRadius:14, padding:"16px", marginBottom:12 }}>
                  ...
                  <div style={{ fontSize:13, color:T.white, lineHeight:1.7, marginBottom:10 }}>
                    A ¥1,200,000 catastrophic event may exceed your Emergency Fund. The answer is <strong style={{ color:T.teal }}>still not to sell investments.</strong>
                  </div>
                  <div style={{ fontSize:13, color:T.offWhite, lineHeight:1.7, marginBottom:12 }}>
                    The real solution is <strong style={{ color:T.purple }}>income protection insurance and disaster coverage</strong> — a layer that sits entirely above the oxygen tank.
                  </div>
              ...
              {!gameOver && (
                <Btn color={T.teal} onClick={next}>
                  {isLast ? "See Mission Results →" : "Next Event (" + (idx+2) + "/10) →"}
                </Btn>
              )}
              {gameOver && (
                <Btn color={T.red} onClick={() => setScreen("result")}>See Mission Debrief →</Btn>
...
function Box({ children, borderColor }) {
  return (
    <div style={{ background:T.navyCard, borderRadius:16, padding:"20px 18px", marginBottom:14, border:"1px solid " + (borderColor || T.navyMid) }}>
      {children}
    </div>
  );
}
...
function Btn({ children, onClick, color, style }) {
  return (
    <button onClick={onClick} style={{ display:"block", width:"100%", padding:"15px", marginTop:14, borderRadius:12, border:"none", background:color, color: color === T.amber ? T.navy : T.white, fontSize:15, fontWeight:800, cursor:"pointer", ...style }}>
      {children}
    </button>
  );
}
